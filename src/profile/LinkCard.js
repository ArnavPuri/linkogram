...
class LinkCard extends React.Component {
    ...
    async handleLinkUpdate(e) {

        this.setState({isLoading: true});
        let response = await axios.post(
            Constants.baseURL + '/updateLink',
            {
                linkID: this.state.linkID,
                token: window.localStorage.getItem('token'),
                newHandle: this.state.handle,
            },
            {
                headers: {
        ...
        if (response.status === 201) {
            //TODO redirect to login if token is expired
            if (response.data.response.code === 'auth/id-token-expired') {
                window.history.pushState('', 'Login again!', `/login`);
            }
        }
    ...
    render() {
        return (
            <div className="flex flex-col">
                ...
                {/*{this.state.isEditing &&*/}
                {/*<button*/}
                {/*    className="bg-red-300 hover:bg-red-500 text-white font-bold py-2 px-4 rounded focus:outline-none focus:shadow-outline text-sm self-end"*/}
                {/*    type="button" onClick={this.handleLinkUpdate}>*/}
                {/*    Save*/}
                {/*</button>*/}

                {/*}*/}
                {this.state.isEditing &&
                <LoadingButton className="text-sm self-end text-center" buttonText="Save"
                               isLoading={this.state.isLoading}
                               onClick={this.handleLinkUpdate}/>
                }
            </div>)
